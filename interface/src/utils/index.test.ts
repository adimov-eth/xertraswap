...
describe('utils', () => {
  describe('#getBscScanLink', () => {
    it('correct for tx', () => {
      expect(getBscScanLink(ChainId.TESTNET, 'abc', 'transaction')).toEqual('https://auroria.explorer.xertra.com/tx/abc')
    })
    it('correct for token', () => {
      expect(getBscScanLink(ChainId.TESTNET, 'abc', 'token')).toEqual('https://auroria.explorer.xertra.com/token/abc')
    })
    it('correct for address', () => {
      expect(getBscScanLink(ChainId.TESTNET, 'abc', 'address')).toEqual('https://auroria.explorer.xertra.com/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getBscScanLink(999999 as ChainId, 'abc', 'address')).toEqual('https://explorer.xertra.com/address/abc')
    })
    it('random chain id', () => {
      expect(getBscScanLink(Number.MAX_SAFE_INTEGER as ChainId, 'abc', 'address')).toEqual(
        'https://explorer.xertra.com/address/abc'
      )
    })
    it('enum', () => {
      expect(getBscScanLink(ChainId.MAINNET, 'abc', 'address')).toEqual('https://explorer.xertra.com/address/abc')
    })
  })
