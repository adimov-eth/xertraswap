...
const getScale =
  (property: ScaleKeys) =>
  ({ scale = scales.MD }: XertraToggleProps) => {
...
export const ToggleStack = styled.div<HandleProps>`
  ...
  .handle {
    background: ${({ theme }) =>
      theme.xertraToggle?.handleBackground || theme.pancakeToggle?.handleBackground || theme.colors.card};
    border-radius: 50%;
    ...
    box-shadow: 0 ${getScale('handleThickness')} 0 ${getScale('handleThickness')}
      ${({ theme }) =>
        theme.xertraToggle?.handleShadow || theme.pancakeToggle?.handleShadow || theme.colors.textDisabled};
  }
`
