...
import { Button, CardBody, AddIcon, Text as UIKitText } from '@xertra/uikit'
...
export default function AddLiquidity({
  ...
  async function onAdd() {
    ...
    if (currencyA === ETHER || currencyB === ETHER) {
      const tokenBIsSTRAX = currencyB === ETHER
      estimate = router.estimateGas.addLiquidityETH
      method = router.addLiquidityETH
      args = [
        wrappedCurrency(tokenBIsSTRAX ? currencyA : currencyB, chainId)?.address ?? '', // token
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/add/${currencyIdA || 'STRAX'}/${newCurrencyIdB}`)
      }
    },
    ...
    <>
      ...
      {pair && !noLiquidity && pairState !== PairState.INVALID ? (
        <AutoColumn style={{ minWidth: '20rem', marginTop: '1rem' }}>
          <MinimalPositionCard showUnwrapped={oneCurrencyIsWSTRAX} pair={pair} />
        </AutoColumn>
      ) : null}
    </>
  )
